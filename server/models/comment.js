...
//   const query = "SELECT * FROM comments WHERE idea_id = $1";
//   const values = [ideaId];
...
const getCommentsByIdeaId = async (ideaId) => {
    const query = "SELECT comments.*, users.user_name FROM comments INNER JOIN users ON comments.user_id = users.id WHERE idea_id = $1";
    const values = [ideaId];
    const { rows } = await db.query(query, values);
    return rows;
  };
...
//   const query =
//     "INSERT INTO comments (user_id, idea_id, comment, comment_date) VALUES ($1, $2, $3, $4) RETURNING *";
